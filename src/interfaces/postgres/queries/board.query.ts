  ...
  implements BoardQuery
{
  ...
  async getByMemberId(
    ...
    const res = await this.client.query(
      `SELECT boards.* FROM boards JOIN board_members ON board_members.board_id = boards.id WHERE board_members.member_id = $1 ${pagination.cursor ? "AND board_members.updated_at < $2" : ""} ORDER BY board_members.updated_at DESC, board_members.board_id DESC LIMIT $${pagination.cursor ? "3" : "2"};`,
      pagination.cursor
  ...
  async getByOwnerId(
    ...
    const res = await this.client.query(
      `SELECT * FROM boards WHERE owner_id = $1 ${pagination.cursor ? "AND updated_at < $2" : ""} ORDER BY updated_at DESC, id DESC LIMIT $${pagination.cursor ? "3" : "2"};`,
      pagination.cursor
